...
###
v8_path = "/home/" + username + "/fuzz/v8/out/normal_build/d8"
dir_path = "/home/" + username + "/fuzz/result/crashes/"

#dir_path = "./result/crashes/duplicates"
###
...
def validate_crushes(dir_path):
    for dir in os.listdir(dir_path):
        crash_dir = os.path.join(dir_path, dir)
        if os.path.isdir(crash_dir):
            file_path = os.path.join(crash_dir, "origin1.js")
            sh_result = execute_file(v8_path, file_path)
            if is_valid(sh_result) == False or sh_result.returncode == 0 or sh_result.returncode == 1:
                remove_dir(crash_dir)

validate_crushes(dir_path)
